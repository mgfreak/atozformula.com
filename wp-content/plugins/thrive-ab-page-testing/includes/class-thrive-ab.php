...
class Thrive_AB {
	...
	const T = 'thrive-ab-page-testing';
	const V = '1.1.3';
	const DB = '1.1';
	...
	public function initiate_dashboard() {
		...
		return $this->_dashboard = Thrive_AB_Dashboard::instance();
	}
	...
	public function remove_admin_bar_tar_button() {
		...
		try {
			$post_id      = $post->ID;
			$is_variation = $post->post_type === Thrive_AB_Post_Types::VARIATION || $post->post_status === Thrive_AB_Post_Status::VARIATION;
	...
	public function determine_variation_template( $template ) {
		...
		if ( $post instanceof WP_Post && $post->post_type === Thrive_AB_Post_Types::VARIATION ) {
	...
	public function view_test_button( $wp_admin_bar ) {
		...
		$args = array(
			'id'    => 'tve_button',
			'title' => __( 'View test details', Thrive_AB::T ),
			'href'  => $test_link,
			'meta'  => array(
	...
	public function maybe_variation( $post ) {
		...
		return $post->post_status === Thrive_AB_Post_Status::VARIATION || $post->post_type === Thrive_AB_Post_Types::VARIATION;
	}
	...
	public static function save_page( $post_ID, $post, $update ) {
		 ...
		 */
		if ( $is_insert && $post->post_status !== Thrive_AB_Post_Status::VARIATION ) {
			return null;
		}
