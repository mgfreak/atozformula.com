...
class Thrive_AB_Meta_Box {
	...
	public function add_meta_logic() {
		...
		if ( get_post_type( $_GET['post'] ) === 'page' ) {
			 ...
			 */
			add_action( 'edit_form_after_title', array( $this, 'include_additional_files' ) );
		}
	}
	...
	public function add_variation_table_meta_box() {
		add_meta_box(
			...
			__( 'Thrive Optimize - A/B Test Overview', Thrive_AB::T ),
	...
	public function add_completed_tests_table_meta_box() {
		add_meta_box(
			...
			__( 'Thrive Completed A/B Tests', Thrive_AB::T ) . ' (' . $this->_completed_tests_number . ')',
