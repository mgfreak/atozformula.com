...
class Thrive_AB_Page extends Thrive_AB_Post {
	...
	public function __construct( $post ) {
		...
		if ( $this->_post->post_type !== 'page' ) {
			throw new Exception( __( 'Provided post is not a page', Thrive_AB::T ) );
		}
	}
	...
	public function save_variation( $model ) {
		...
		$model = array_merge( array(
			...
			'post_type'   => 'page',
			'post_parent' => $this->_post->ID,
			'post_title'  => __( 'Variation', Thrive_AB::T ),
		), $model );
		...
		if ( is_wp_error( $post ) || $post === 0 ) {
			throw new ErrorException( __( 'Variation could not be saved', Thrive_AB::T ) );
		}
