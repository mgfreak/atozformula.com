...
class Thrive_AB_Ajax_Controller {
	...
	public function handle() {
		...
		if ( ! check_ajax_referer( Thrive_AB_Ajax::NONCE_NAME, 'nonce', false ) ) {
			$this->error( __( 'Invalid request.', Thrive_AB::T ) );
		}
		...
		if ( ! method_exists( $this, $function ) ) {
			$this->error( sprintf( __( 'Method %s not implemented', Thrive_AB::T ), $function ) );
		}
	...
	protected function tests_action( $method, $model ) {
		...
		switch ( $method ) {
			...
			case 'PUT':
				try {
					if ( ! empty( $model['goal_pages'] ) ) {
						$model['goal_pages'] = serialize( $model['goal_pages'] );
					}
	...
	protected function testsforadmin_action() {
		...
		$goals = array(
			'monetary' => __( 'Revenue', Thrive_AB::T ),
			'visits'   => __( 'Goal Page Visit', Thrive_AB::T ),
			'optins'   => __( 'Subscriptions', Thrive_AB::T ),
		);
	...
	protected function deletecompletedtestadmin_action() {
		$return = array(
			'success' => 0,
			'text'    => __( 'There was an error in the process.', Thrive_AB::T ),
		);
		...
		if ( ! empty( $id ) && ! empty( $page_id ) ) {
			...
			$return['success'] = 1;
			$return['text']    = __( 'The test was deleted successfully!', Thrive_AB::T );
		}
