...
class Thrive_AB_Event_Manager {
	...
	public static function check_thank_you_page( $post ) {
		...
		foreach ( $tests as $test ) {
			...
			if ( ! empty( $goal_pages ) && ! empty( $goal_pages[ $post->ID ] ) ) {
				Thrive_AB_Event_Manager::do_conversion( $test->id, $test->page_id, $impression_variation, $goal_pages[ $post->ID ] );
				continue;
			}
		}
	...
	public static function count_visit( $model ) {
		...
		$test_item->init_by_filters( array(
	...
	public static function check_auto_winner( $event ) {
		...
		if ( ! $test->auto_win_enabled || $test->status !== 'running' ) {
			return null;
		}
