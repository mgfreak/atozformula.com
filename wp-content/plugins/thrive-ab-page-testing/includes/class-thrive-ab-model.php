...
abstract class Thrive_AB_Model {
	...
	public function __get( $key ) {
		...
		if ( method_exists( $this, $key ) ) {
			...
			if ( $key == 'goal_pages' ) {
				$value = isset( $this->_data[ $key ] ) ? unserialize( $this->_data[ $key ] ) : null;
			} else {
	...
	public function init() {
		...
		if ( ! $this->id ) {
			throw new Exception( __( 'Invalid model id', Thrive_AB::T ) );
		}
	...
	public function save() {
		...
		if ( ! $this->is_valid() ) {
			throw new Exception( __( sprintf( 'Invalid model %s with data: %s', get_class( $this ), var_export( $this->_data, true ) ), Thrive_AB::T ) );
		}
		...
		if ( is_wp_error( $saved ) || $saved === false ) {
			throw new Exception( __( 'Model could not be saved', Thrive_AB::T ) );
		}
	...
	public function delete() {
		if ( ! is_numeric( $this->id ) ) {
			throw new Exception( __( 'Invalid input for delete', Thrive_AB::T ) );
		}
