...
class Thrive_AB_Report_Manager {
	...
	public function get_test_chart_data( $filters = array() ) {
		...
		foreach ( $test_items as $item ) {
			$variations[ $item->variation_id ] = array( 'name' => html_entity_decode( $item->title ), 'revenue' => $item->revenue );
		}
		...
		$return = array(
			'ID'              => $filters['test_id'],
			'title'           => __( 'Total conversions over time', Thrive_AB::T ),
			'data'            => $chart_data,
			'x_axis'          => $dates,
			'y_axis'          => __( 'Total visits', Thrive_AB::T ),
			'total_over_time' => $test_data['type'] === 'conversion_rate' ? $this->conversion_rate( $total_impressions, $total_conversions ) : $total_over_time,
			'test_type_txt'   => '',
		);

		if ( $test_data['type'] === 'monetary' ) {
			$return['title']         = __( 'Total revenue over time', Thrive_AB::T );
			$return['y_axis']        = __( 'Total revenue', Thrive_AB::T ) . ' $';
			$return['test_type_txt'] = '$';
		} elseif ( $test_data['type'] === 'optins' ) {
			$return['title']  = __( 'Total subscriptions over time', Thrive_AB::T );
			$return['y_axis'] = __( 'Total subscriptions', Thrive_AB::T );
		} elseif ( $test_data['type'] === 'conversion_rate' ) {
			$return['title']         = __( 'Page conversion rate', Thrive_AB::T );
			$return['y_axis']        = __( 'Conversion rate (%)', Thrive_AB::T );
			$return['test_type_txt'] = '%';
		}
