...
class Thrive_AB_Meta_Box_Variations_Table extends WP_List_Table {
	...
	public function get_columns() {
		...
		$columns = array(
			'post_title'          => __( 'Name', Thrive_AB::T ),
			'traffic'             => __( 'Traffic', Thrive_AB::T ),
			'impressions'         => __( 'Unique Visitors', Thrive_AB::T ),
			'conversions'         => __( 'Conversions', Thrive_AB::T ),
			'conversions_rate'    => __( 'Conversions Rate', Thrive_AB::T ),
			'revenue'             => __( 'Revenue', Thrive_AB::T ),
			'revenue_visitor'     => __( 'Revenue per visitor', Thrive_AB::T ),
			'improvement'         => __( 'Improvement', Thrive_AB::T ),
			'chance_to_beat_orig' => __( 'Chance to Beat Original', Thrive_AB::T ),
		);

		if ( is_array( $this->_page_tests ) && count( $this->_page_tests ) && $this->_page_tests[0]['type'] === 'optins' ) {
			$columns['conversions']      = __( 'Subscriptions', Thrive_AB::T );
			$columns['conversions_rate'] = __( 'Subscriptions Rate', Thrive_AB::T );
		}
	...
	public function no_items() {
		echo __( 'There are no Thrive A/B Test variations for this page', Thrive_AB::T );
	}
...
//		}
	...
	public function _column_improvement( $item, $classes, $data, $primary ) {
		...
		if ( $item->is_control() ) {
			...
			$html    .= "<td class='$classes' $data colspan='2'>[ " . __( 'This is the control', Thrive_AB::T ) . ' ]</td>';
			echo $html;
