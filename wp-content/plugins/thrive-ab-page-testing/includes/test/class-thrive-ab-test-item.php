...
class Thrive_AB_Test_Item extends Thrive_AB_Model {
	...
	public function init_by_filters( $filters = array() ) {
		if ( empty( $filters ) ) {
			throw new Exception( __( 'Invalid filters', Thrive_AB::T ) );
		}
	...
	protected function _prepare_data() {
		...
		//todo: refactor those IFs wtf
		if ( isset( $data['items'] ) ) {
			unset( $data['items'] );
		}

		if ( isset( $data['preview_link'] ) ) {
			unset( $data['preview_link'] );
		}
		...
		if ( isset( $data['revenue_visitor'] ) ) {
			unset( $data['revenue_visitor'] );
		}
		...
		if ( isset( $data['chance_to_beat_orig'] ) ) {
			unset( $data['chance_to_beat_orig'] );
		}
